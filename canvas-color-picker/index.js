...
function togglePicker() {
  ...
  if (isPickerSelected) {
    ...
    document.querySelector(".svg-container").classList.remove("svg-inactive");
    document.querySelector(".svg-container").classList.add("svg-active");
  } else {
    body.style.cursor = "auto";
    document.querySelector(".svg-container").classList.add("svg-inactive");
    document.querySelector(".svg-container").classList.remove("svg-active");
  }
}

document.getElementById("pickMe").onclick = (e) => {
  togglePicker();
  e.stopPropagation();
};
...
myCanvas.onmousemove = (e) => {
  if (isPickerSelected) {
    ...
    const hex = `#${rgb[0].toString(16)}${rgb[1].toString(16)}${rgb[2].toString(
      16
    )}`;
    document.querySelector(".preview-circle img").style.background = hex;
    document.querySelector(".preview-text").style.color = hex;
  }
  e.stopPropagation();
};
...
myCanvas.onclick = (e) => {
  if (isPickerSelected) {
    ...
    document.getElementById("palette-code").style.color = hex;
    document.getElementById("palette-code").innerHTML = hex;
  }
  e.stopPropagation();
};
